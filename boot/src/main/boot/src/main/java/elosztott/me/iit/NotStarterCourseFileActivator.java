...
public class NotStarterCourseFileActivator {
	...
	public ExecutionReport putIntoFile(Message<CourseModel> message){
		...
		if(manager.containsCourse(course)){
			return generateError("Course already exists");
		}
		Date date=(Date) message.getHeaders().get("additionDate");
		writeToFile(course,date);
		return new ExecutionReport("Not a starter course!", course);
	}
	
private void writeToFile(CourseModel course, Date date) {
		...
		try {
			...
			outputNotStarter.append(course.getCourseName()+","+course.getCourseCredit()+","+date.toString());
